...
function scaleContext(scaleName, transposeSemi = 0) {
  ...
  return {
    ...
    steps: MODE[mode].scale, arpSteps: MODE[mode].arp,
    slotOfPc,
...
function compose(segments) {
  ...
  for (const seg of segments) {
    ...
    for (let i = from; i < seg.pcs.length; i++) {
      pcs.push(seg.pcs[i]); slots.push(seg.slots[i]);
      steps.push(seg.steps[i]); phases.push(seg.phases[i]);
    }
  }
...
const tone = (ctx, i, oct = 0) =>
  ({ pc: ctx.arp[i], slot: ctx.slotOfPc[ctx.arp[i]], step: ctx.arpSteps[i] + 12 * oct });
...
const patterns = {
  scale: {
    ...
    double: ctx => {
      const p = compose([scaleUpSeg(ctx), scaleUp2Seg(ctx), scaleDown2Seg(ctx),
                         scaleDownSeg(ctx), arp2Seg(ctx)]);
